...
## 1. 一个 Myself，代码与个人数据分离

Myself 的代码与个人数据分开，运行时有三个明确职责：

- **Myself 仓库**：服务所有用户的结构、协议、模板和工具，绑定配置保存在被 Git 忽略的 `.myself/local.json`。
- **用户 Vault**：每位用户唯一的长期个人知识存储，同时包含原始资料、整理后的理解、正式状态和历史。
- **Obsidian 界面**：直接阅读和编辑 Vault 中的本地文件，不负责决定内容是否真实或已经获得批准。

Myself 在运行时连接用户指定的 Vault，并把经过验证和授权的结果写回其中。仓库目录不建立第二套个人知识。聊天、网页、日历、项目管理器和可穿戴设备只是输入源，不能自动覆盖正式状态。

```mermaid
flowchart LR
    M["Myself：方法、Agent 与工具"] -->|收录、整理、验证、写入| V["用户 Vault：个人知识与状态"]
    V -->|检索背景、证据与反馈| M
    O["Obsidian"] <-->|阅读与编辑| V
    V -->|经验证的通用使用反馈| M
```

这是两个循环：日常使用更新个人知识；使用验证推动 Myself 改进。仓库与用户 Vault 不自动同步，Myself 更新也不自动迁移已有 Vault。具体操作见[知识系统](knowledge-system.md)。
...
规则改进从一次具体使用问题开始，明确候选改动、试用范围、效果指标与复查时间。由实际结果决定采用、调整或撤回；采用后才考虑提炼为 Myself 的通用规则。Agent 的自评可以提出问题，不能作为证明规则有效的唯一证据。
